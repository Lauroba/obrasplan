...
# deploy-movimientos-foto-v2.ps1
# 1. Modal Nuevo movimiento: elimina campo Obra (opcional)
# 2. Foto de articulo: mas grande y centrada en el modal
# 3. FotoArticulo: componente reutilizable con popup hover
#    Usado en listado de articulos, movimientos y traslado masivo
...
$ok2 = Select-String -Path "src\app\almacen\movimientos\page.tsx" -Pattern "FotoArticulo" -Quiet
$ok3 = Select-String -Path "src\app\almacen\movimientos\page.tsx" -Pattern "obra_id.*movForm\|movForm.*obra_id" -Quiet
if ($ok1) { Write-Host "    OK: FotoArticulo.tsx creado" -ForegroundColor Green }
else { Write-Host "    ERROR: falta FotoArticulo.tsx" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: movimientos usa FotoArticulo" -ForegroundColor Green }
else { Write-Host "    ERROR: movimientos sin FotoArticulo" -ForegroundColor Red }
if (-not $ok3) { Write-Host "    OK: campo Obra eliminado del modal movimiento" -ForegroundColor Green }
else { Write-Host "    AVISO: revisar campo obra en movimientos" -ForegroundColor Yellow }
Write-Host ""
Write-Host '  git add src\app\almacen\movimientos\page.tsx src\app\almacen\articulos\page.tsx src\components\shared\FotoArticulo.tsx'
Write-Host '  git commit -m "feat: modal movimiento sin obra, foto articulo grande, popup hover miniaturas"'
Write-Host '  git push'

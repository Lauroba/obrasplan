...
# fix-planificador-conflictos-v2.ps1
# Fix del bug visual: todos los avatares aparecian en rojo.
# Causa: conflictResources guardaba "tipo|id" sin fecha, y al comprobar
# si un recurso tenia conflicto se ignoraba el dia -> marcaba en rojo
# ese recurso en TODOS los dias de la semana.
# Fix: conflictResources guarda "tipo|id|fecha" y ObraCell comprueba
# con el dateStr de esa celda concreta.
...
Write-Host "" ; Write-Host "==> Fix conflictos planificador v2" -ForegroundColor Cyan
...
$ok = Select-String -Path "src\app\planificacion\page.tsx" -Pattern 'has\(`\$\{rkey\}\|\$\{dateStr\}' -Quiet
if ($ok) { Write-Host "    OK: conflicto comprobado con tipo|id|fecha" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "fix: planificador conflicto - solo marca el recurso en el dia exacto"'
Write-Host '  git push'

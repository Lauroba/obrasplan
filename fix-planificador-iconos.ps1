...
# fix-planificador-iconos.ps1
# Elimina los iconos/botones de maquinaria y material de los filtros
# del planificador y del select del modal de asignacion manual.
...
$ok = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "maquinaria.*icon|icon.*Wrench" -Quiet
if (-not $ok) { Write-Host "    OK: iconos de maquinaria eliminados" -ForegroundColor Green }
else { Write-Host "    AVISO: quedan referencias a maquinaria" -ForegroundColor Yellow }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "fix: eliminar iconos maquinaria y material del planificador"'
Write-Host '  git push'

...
# fix-partes-build-error.ps1
# Corrige el error de compilacion: input file duplicado en el JSX
# + simplifica el email al firmar (sin confirm, sin calcular toEmail)
# Todo lo demas ya estaba correcto en el repo:
#   - Materiales eliminados
#   - Firma cliente al final
...
Write-Host "" ; Write-Host "==> Fix build error partes" -ForegroundColor Cyan
...
Write-Host "    OK: page.tsx actualizado" -ForegroundColor Green
...
$ok3 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "safeName" -Quiet
$ok4 = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "confirm\(" -Quiet)
$content = Get-Content -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Raw
$inputCount = ([regex]::Matches($content, 'type="file"')).Count
if ($ok1) { Write-Host "    OK: sin materiales" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: firma cliente al final" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: safeName en upload" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($ok4) { Write-Host "    OK: sin confirm()" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($inputCount -eq 1) { Write-Host "    OK: solo 1 input file (sin duplicado)" -ForegroundColor Green }
else { Write-Host "    ERROR: $inputCount inputs file" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add "src\app\partes\[id]\page.tsx"'
Write-Host '  git commit -m "fix: build error - input file duplicado + email simplificado"'
Write-Host '  git push'

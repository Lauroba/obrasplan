...
# fix-asignacion-sin-finde.ps1
# Al asignar un recurso con rango de fechas en el planificador,
# los sabados (6) y domingos (0) se saltan automaticamente.
# Solo afecta al modal de asignacion manual con fecha inicio/fin.
# El drag & drop de dia individual no se ve afectado (ya solo
# muestra dias laborables en el grid).
...
$ok = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "Saltar fines de semana" -Quiet
if ($ok) { Write-Host "    OK: skip fines de semana en asignacion manual" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "fix: asignacion manual excluye fines de semana"'
Write-Host '  git push'

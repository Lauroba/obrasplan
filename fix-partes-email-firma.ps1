...
# fix-partes-email-firma.ps1
# Al firmar un parte, se envia automaticamente por email sin pedir confirmacion.
# lauroba.eneko@gmail.com recibe SIEMPRE una copia (CC), independientemente
# de si la obra tiene o no email de contacto configurado.
#
# Cambios:
# 1. route.ts (API): ADMIN_CC = "lauroba.eneko@gmail.com" se añade siempre
#    al array CC antes de enviar, aunque ya estuviera en cc_emails de settings.
# 2. partes/[id]/page.tsx: al firmar, se llama a la API de email automaticamente
#    sin confirm() ni alert(). Si la obra tiene email -> destinatario principal.
#    Si no tiene -> lauroba.eneko@gmail.com es el destinatario principal.
#    El admin siempre recibe copia via CC de la route.
...
Write-Host "" ; Write-Host "==> Fix email automatico al firmar parte" -ForegroundColor Cyan
...
$ok1 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\api\partes\email\route.ts") -Pattern "ADMIN_CC" -Quiet
$ok2 = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "Parte firmado" -Quiet)
$ok3 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "lauroba.eneko" -Quiet
if ($ok1) { Write-Host "    OK: CC fijo en route.ts" -ForegroundColor Green }
else { Write-Host "    ERROR: falta ADMIN_CC" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: sin confirm() al firmar" -ForegroundColor Green }
else { Write-Host "    WARN: puede quedar confirm antiguo" -ForegroundColor Yellow }
if ($ok3) { Write-Host "    OK: lauroba.eneko en page.tsx" -ForegroundColor Green }
else { Write-Host "    ERROR: falta email admin" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\api\partes\email\route.ts'
Write-Host '  git add "src\app\partes\[id]\page.tsx"'
Write-Host '  git commit -m "feat: partes - email automatico al firmar, CC fijo admin"'
Write-Host '  git push'

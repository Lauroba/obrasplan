...
# fix-planificador-sinasignar.ps1
# Implementa la fila SIN ASIGNAR virtual en el planificador:
#   - Fila fija siempre primera, calcula automaticamente RRHH y vehiculos
#     no asignados cada dia (dias laborables), sin depender de ninguna obra.
#   - Elimina los flags flag_rrhh_sin_asignar / flag_vehiculo_sin_asignar
#     de la ficha de obra (la seccion "Configuracion especial" desaparece).
#   - Las columnas en BD se mantienen pero se ignoran desde frontend.
#   - Drag desde SIN ASIGNAR a obra real: crea asignacion real.
#   - Drag desde obra a SIN ASIGNAR: elimina la asignacion de ese dia.
#   - La fila SIN ASIGNAR no se puede reordenar, archivar ni eliminar.
...
$ok1 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "SIN_ASIGNAR_ID" -Quiet
$ok2 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "SinAsignarRow" -Quiet
$ok3 = Select-String -Path "src\app\obras\nueva\page.tsx" -Pattern "Configuracion especial|Configuraci" -Quiet
if ($ok1) { Write-Host "    OK: SIN_ASIGNAR_ID definido en planificador" -ForegroundColor Green }
else { Write-Host "    ERROR: falta SIN_ASIGNAR_ID" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: SinAsignarRow presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta SinAsignarRow" -ForegroundColor Red }
if (-not $ok3) { Write-Host "    OK: seccion Configuracion especial eliminada de obras/nueva" -ForegroundColor Green }
else { Write-Host "    AVISO: aun hay referencias a Configuracion especial (revisar)" -ForegroundColor Yellow }
Write-Host ""
Write-Host "No se necesita migracion SQL." -ForegroundColor Cyan
Write-Host "Las columnas flag_rrhh_sin_asignar y flag_vehiculo_sin_asignar"
Write-Host "se mantienen en BD pero se ignoran desde el frontend."
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx src\app\obras\nueva\page.tsx'
Write-Host '  git commit -m "feat: fila SIN ASIGNAR virtual en planificador, eliminar flags de ficha de obra"'
Write-Host '  git push'

...
# fix-tab-almacen-obra.ps1
# Garantiza que la pestana Almacen aparece en la ficha de obra.
# El codigo ya estaba correcto en el repo - este script reescribe
# los archivos para forzar un redeploy limpio en Vercel.
...
Write-Host "" ; Write-Host "==> Reescribiendo archivos" -ForegroundColor Cyan
...
$ok1 = Select-String -Path "src\app\obras\[id]\obra-detail.tsx" -Pattern "almacen.*label.*Almac" -Quiet
$ok2 = Test-Path -LiteralPath (Join-Path $RepoPath "src\app\obras\[id]\almacen\page.tsx")
if ($ok1) { Write-Host "    OK: pestana Almacen presente en obra-detail" -ForegroundColor Green }
else { Write-Host "    ERROR: pestana no encontrada" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: pagina /obras/[id]/almacen existe" -ForegroundColor Green }
else { Write-Host "    ERROR: falta pagina almacen de obra" -ForegroundColor Red }

Write-Host ""
Write-Host "COMPROBACION SUPABASE (ejecutar en SQL Editor si la pestana sale vacia):" -ForegroundColor Yellow
Write-Host "  SELECT id, nombre FROM almacenes WHERE es_almacen_obra = true LIMIT 5;"
Write-Host "  SELECT * FROM v_stock_actual LIMIT 5;"
Write-Host ""
Write-Host '  git add -A'
Write-Host '  git commit -m "fix: forzar redeploy - pestana Almacen en ficha de obra"'
Write-Host '  git push'
Write-Host ""
Write-Host "Despues de git push:" -ForegroundColor Cyan
Write-Host "  1. Ve a Vercel"
Write-Host "  2. Espera a que el build termine"
Write-Host "  3. Pulsa el menu de 3 puntos del deployment"
Write-Host "  4. Selecciona 'Promote to Production'"

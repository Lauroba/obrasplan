...
# fix-georadar-rename.ps1
# 1. Elimina "Georadar V2" del menu, rutas y permisos
# 2. Renombra "Interpretacion de Georradar" a "Georadar" en:
#    - Menu lateral (Sidebar)
#    - Titulo interno de la pagina
#    - Pantalla de Configuracion -> Roles y permisos
# La carpeta /aplicaciones/georadar-v2 se elimina del repo manualmente
# o se puede ignorar (Next.js no la mostrara si no hay entrada en el Sidebar)
...
Write-Host "" ; Write-Host "==> Aplicando cambios" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "==> Eliminando carpeta georadar-v2 del repo" -ForegroundColor Cyan
$v2path = Join-Path $RepoPath "src\app\aplicaciones\georadar-v2"
if (Test-Path $v2path) {
    Remove-Item -Recurse -Force $v2path
    Write-Host "    OK: carpeta georadar-v2 eliminada" -ForegroundColor Green
} else {
    Write-Host "    INFO: carpeta georadar-v2 no existia" -ForegroundColor Gray
}

Write-Host ""
Write-Host "==> Verificando" -ForegroundColor Cyan
$ok1 = Select-String -Path "src\components\layout\Sidebar.tsx" -Pattern '"Georadar"' -Quiet
$ok2 = Select-String -Path "src\components\layout\Sidebar.tsx" -Pattern 'georadar-v2' -Quiet
$ok3 = !(Test-Path (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2"))
if ($ok1) { Write-Host "    OK: Sidebar muestra 'Georadar'" -ForegroundColor Green }
else { Write-Host "    ERROR Sidebar" -ForegroundColor Red }
if (-not $ok2) { Write-Host "    OK: Georadar V2 eliminado del Sidebar" -ForegroundColor Green }
else { Write-Host "    ATENCION: queda referencia a georadar-v2 en Sidebar" -ForegroundColor Yellow }
if ($ok3) { Write-Host "    OK: carpeta georadar-v2 no existe" -ForegroundColor Green }
else { Write-Host "    INFO: carpeta georadar-v2 aun presente" -ForegroundColor Yellow }
Write-Host ""
Write-Host '  git add -A'
Write-Host '  git commit -m "fix: eliminar Georadar V2, renombrar a Georadar"'
Write-Host '  git push'
Write-Host ""
Write-Host "Menu lateral resultante:" -ForegroundColor Cyan
Write-Host "  ...otras entradas..."
Write-Host "  Georadar  ->  /aplicaciones/georadar"
Write-Host "  (Georadar V2 ya no aparece)"

...
# fix-toggle-access-errores.ps1
#
# Corrige el boton "Desactivar acceso" que no hacia nada visible al fallar:
#   - API: toggle_access usaba .single() que falla silenciosamente si no
#     encuentra exactamente 1 fila en users por recurso_id. Ahora usa
#     .maybeSingle() y devuelve errores claros en cada paso (actualizar
#     perfil, banear en Auth, actualizar recurso).
#   - Frontend: handleToggleAccess y handleToggleAccessInModal ahora
#     comprueban la respuesta real de la API. Si falla, se muestra el
#     mensaje de error dentro del modal en vez de fallar en silencio.
#
# CASO JON (ortegalagojon@gmail.com): probablemente su recurso_id en
# recursos_humanos no coincide con el recurso_id guardado en users tras
# la restauracion manual. El error que aparecera ahora te dira exactamente
# si es por eso.
...
$ok1 = Select-String -Path "src\app\api\users\route.ts" -Pattern "maybeSingle" -Quiet
$ok2 = Select-String -Path "src\app\maestros\recursos-humanos\page.tsx" -Pattern "res.ok" -Quiet
if ($ok1) { Write-Host "    OK: API usa maybeSingle con manejo de errores" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: frontend comprueba respuesta" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\api\users\route.ts src\app\maestros\recursos-humanos\page.tsx'
Write-Host '  git commit -m "fix: toggle_access con manejo de errores robusto, mostrar fallos en UI"'
Write-Host '  git push'

...
# fix-eliminar-usuario.ps1
# Corrige el error "Database error deleting user":
#   1. Anade comprobacion de 4 tablas que tenian FK sin ON DELETE explicito
#      (checklists, checklist_items, planificador_notas, conflictos_revisados,
#      tareas.completada_by, georadar_pasadas) y que bloqueaban el borrado
#      en Auth con un error generico.
#   2. Invierte el orden: ahora se borra primero de Auth (lo mas restrictivo).
#      Si falla, el usuario queda intacto (no se borra nada de la app).
#      Solo si Auth se borra con exito se borra de public.users y recursos_humanos.
#      Esto evita el estado inconsistente "eliminado de la app pero no de Auth".
...
$ok = Select-String -Path "src\app\api\users\route.ts" -Pattern "checklist_items" -Quiet
if ($ok) { Write-Host "    OK: comprobaciones adicionales presentes" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\api\users\route.ts'
Write-Host '  git commit -m "fix: eliminar usuario - comprobar mas tablas, orden Auth primero"'
Write-Host '  git push'

...
# fix-obras-listado-roles.ps1
# El listado de obras filtraba por asignaciones a cualquier usuario no-admin,
# incluyendo Jefe de Almacen, Oficina, etc. que deben ver TODAS las obras.
# Fix: solo aplicar el filtro por asignaciones a operarios puros
# (usuarios sin permiso de crear ni editar obras).
...
$ok = Select-String -Path "src\app\obras\page.tsx" -Pattern "esSoloOperario" -Quiet
if ($ok) { Write-Host "    OK: filtro obras por rol correcto" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\obras\page.tsx'
Write-Host '  git commit -m "fix: listado obras muestra todas para roles con permiso, solo filtra operarios puros"'
Write-Host '  git push'

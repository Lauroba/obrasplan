...
# fix-obras-archivar-listado.ps1
# Anade icono de archivar/restaurar en cada fila del listado de obras.
# - Primer clic: muestra "Confirmar?" con el icono en ambar
# - Segundo clic: archiva/restaura la obra
# - Clic en cualquier otro sitio: cancela la confirmacion
...
$ok = Select-String -Path "src\app\obras\page.tsx" -Pattern "handleArchivar" -Quiet
if ($ok) { Write-Host "    OK: icono archivar en listado" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\obras\page.tsx'
Write-Host '  git commit -m "feat: archivar obras desde el listado con confirmacion inline"'
Write-Host '  git push'

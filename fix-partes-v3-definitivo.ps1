...
# fix-partes-v3-definitivo.ps1
# 3 cambios en 2 archivos:
#
# 1. EMAIL: siempre y SOLO a lauroba.eneko@gmail.com
#    - route.ts: ignora toEmail, usa FIXED_TO hardcodeado
#    - Sin CC, sin logica condicional, sin dependencia del email de la obra
#    - page.tsx: envia solo parteId, sin calcular toEmail
#    - Logs en consola Vercel para verificar: buscar [partes/email]
#
# 2. MATERIALES: seccion "Trabajos / Materiales" eliminada del parte
#    Los datos siguen en BD (parte_lineas) pero no se muestran en la UI
#
# 3. FIRMA: solo firma del cliente, al final del parte
#    - Eliminada la firma del Responsable
#    - La firma del cliente se mueve al final, despues de Audios
#    - El boton Firmar solo requiere firma del cliente (no la del responsable)
...
Write-Host "" ; Write-Host "==> Aplicando fix partes definitivo" -ForegroundColor Cyan
...
$ok1 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\api\partes\email\route.ts") -Pattern "FIXED_TO" -Quiet
$ok2 = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "Trabajos / Materiales" -Quiet)
$ok3 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "Firma del cliente" -Quiet
$ok4 = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "Responsable —" -Quiet)
if ($ok1) { Write-Host "    OK: FIXED_TO en route.ts" -ForegroundColor Green }
else { Write-Host "    ERROR: falta FIXED_TO" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: materiales eliminados" -ForegroundColor Green }
else { Write-Host "    WARN: pueden quedar materiales" -ForegroundColor Yellow }
if ($ok3) { Write-Host "    OK: firma cliente al final" -ForegroundColor Green }
else { Write-Host "    ERROR: falta seccion firma" -ForegroundColor Red }
if ($ok4) { Write-Host "    OK: firma responsable eliminada" -ForegroundColor Green }
else { Write-Host "    WARN: puede quedar firma responsable" -ForegroundColor Yellow }
Write-Host ""
Write-Host "VERIFICAR ENTREGA EMAIL:" -ForegroundColor Cyan
Write-Host "  Vercel Dashboard -> proyecto -> Logs (Functions)"
Write-Host "  Buscar: [partes/email] Enviando parte"
Write-Host "  Buscar: [partes/email] OK - emailId:"
Write-Host ""
Write-Host '  git add src\app\api\partes\email\route.ts "src\app\partes\[id]\page.tsx"'
Write-Host '  git commit -m "fix: partes - email solo admin, sin materiales, firma solo cliente"'
Write-Host '  git push'

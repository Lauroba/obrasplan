...
# fix-logs-entidades.ps1
# 1. Entidades en logs: cargadas dinamicamente desde BD (todas las que existan)
#    ordenadas alfabeticamente, no solo las de la lista fija
# 2. Multi-seleccion: dropdown con checkboxes para filtrar varias entidades a la vez
# 3. Cierre automatico del dropdown al hacer clic fuera
...
$ok = Select-String -Path "src\app\logs\page.tsx" -Pattern "filterEntidades" -Quiet
if ($ok) { Write-Host "    OK: multi-seleccion de entidades" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\logs\page.tsx'
Write-Host '  git commit -m "feat: logs filtro multi-entidad dinamico desde BD"'
Write-Host '  git push'

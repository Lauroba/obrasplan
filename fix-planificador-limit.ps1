...
# fix-planificador-limit.ps1
# Corrige el fetch de asignaciones: sin .limit() explicito Supabase
# devuelve maximo 1000 filas. Con mas de 1000 asignaciones en BD, el
# planificador no las ve todas y los inserts nuevos tampoco aparecen.
...
$ok = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "limit.10000." -Quiet
if ($ok) { Write-Host "    OK: limit 10000 anadido" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "fix: planificador asignaciones limit 10000"'
Write-Host '  git push'

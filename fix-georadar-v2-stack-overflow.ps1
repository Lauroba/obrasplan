...
# fix-georadar-v2-stack-overflow.ps1
# CAUSA RAIZ: Maximum call stack size exceeded al cargar CSV de parametros
#
# El bug estaba en MapsPanelV2.tsx:
#   1. HeatOverlay (clase definida DENTRO del useEffect) capturaba store.anoms
#      en closure. Cada vez que Google Maps llamaba a draw(), modificaba
#      el canvas, lo que podia disparar un repaint del mapa -> draw() otra
#      vez -> bucle infinito -> stack overflow.
#   2. El useEffect de inicializacion del mapa (deps: [mapsReady]) leia
#      store.gps en closure pero el store podia cambiar sin que el efecto
#      se re-ejecutara, creando inconsistencias.
#
# FIX DEFINITIVO:
#   - Eliminado HeatOverlay completamente
#   - Heatmap dibujado SOLO en el listener "idle" del mapa
#   - anomsRef y viewModeRef: refs que el listener lee directamente
#     (siempre datos actuales sin necesitar reconfigurarse)
#   - mapInited (ref booleana): el mapa solo se inicializa UNA vez
#     aunque store.gps cambie -> no mas re-creacion del mapa
#   - useEffect de marcadores separado del de inicializacion
#     con deps correctas: [store.anoms, store.gps, filters, viewMode]
...
Write-Host "" ; Write-Host "==> Fix stack overflow MapsPanelV2" -ForegroundColor Cyan
...
Write-Host "    OK: MapsPanelV2.tsx actualizado" -ForegroundColor Green

$ok1 = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern 'HeatOverlay' -Quiet)
$ok2 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern 'mapInited' -Quiet
$ok3 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern 'anomsRef' -Quiet
if ($ok1) { Write-Host "    OK: HeatOverlay eliminado" -ForegroundColor Green }
else { Write-Host "    ERROR: queda HeatOverlay" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: mapInited flag presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta mapInited" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: anomsRef presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta anomsRef" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx'
Write-Host '  git commit -m "fix: Georadar V2 stack overflow - eliminar HeatOverlay closure, usar refs"'
Write-Host '  git push'

...
# fix-planificador-confirmar-eliminar.ps1
# 1. MODAL DE CONFIRMACION al eliminar asignacion:
#    Clic en avatar -> modal "Eliminar asignacion de [nombre]?" con boton
#    Cancelar / Si, eliminar. Funciona para TODOS los usuarios.
#
# 2. GUARD DE PERMISOS en eliminar:
#    handleRemove comprueba puedeAsignar antes de abrir el modal.
#    Si el usuario no tiene permiso asignar en planificacion -> ignorado.
#    Los avatares visualmente muestran cursor:default si no puede eliminar.
#    Sergio (Jefe Almacen) sin permiso asignar -> NO puede eliminar.
...
Write-Host "" ; Write-Host "==> Fix planificador: confirmar eliminar + permisos" -ForegroundColor Cyan
...
Write-Host "    OK: planificacion/page.tsx actualizado" -ForegroundColor Green

$ok1 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "removeConfirm" -Quiet
$ok2 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "handleRemoveConfirmed" -Quiet
$ok3 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "puedeEliminar" -Quiet
$ok4 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "Si, eliminar" -Quiet
if ($ok1) { Write-Host "    OK: estado removeConfirm presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta removeConfirm" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: handleRemoveConfirmed presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta handleRemoveConfirmed" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: puedeEliminar en avatares" -ForegroundColor Green }
else { Write-Host "    ERROR: falta puedeEliminar" -ForegroundColor Red }
if ($ok4) { Write-Host "    OK: modal con boton Si, eliminar" -ForegroundColor Green }
else { Write-Host "    ERROR: falta boton modal" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "feat: planificador - confirmar eliminar + guard permisos"'
Write-Host '  git push'

...
# deploy-eliminar-usuario.ps1
# Permite eliminar usuarios completamente (app + Supabase Auth) desde
# el maestro de Recursos Humanos.
#
# Reglas de seguridad implementadas:
#   - Si el usuario tiene historial vinculado (partes diarios creados,
#     movimientos de almacen, presencia en partes, asignaciones en el
#     planificador), el borrado se BLOQUEA con un mensaje claro.
#     En ese caso hay que usar "Desactivar acceso" en su lugar.
#   - Si NO tiene historial, se borra de: auth.users (servidor), public.users
#     y recursos_humanos. Queda registrado en audit_log.
#
# Desactivar (boton existente "Activo/Inactivo") sigue funcionando igual
# y es la opcion recomendada para usuarios con historial.
...
$ok1 = Select-String -Path "src\app\api\users\route.ts" -Pattern 'action === "delete"' -Quiet
$ok2 = Select-String -Path "src\app\maestros\recursos-humanos\page.tsx" -Pattern "handleDelete" -Quiet
if ($ok1) { Write-Host "    OK: accion delete en API" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: handleDelete en frontend" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\api\users\route.ts src\app\maestros\recursos-humanos\page.tsx'
Write-Host '  git commit -m "feat: eliminar usuarios completamente (app + Auth servidor) con proteccion de historial"'
Write-Host '  git push'

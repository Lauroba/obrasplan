...
# fix-partes-llave-faltante.ps1
# Causa raiz del error de compilacion:
# El archivo page.tsx tenia 212 llaves { y 211 llaves }
# Faltaba la } que cierra la funcion ParteDetallePage()
# Next.js/SWC detectaba esto como token inesperado en el return
# porque tecnicamente el return estaba "dentro" de un contexto invalido.
# Este script escribe el archivo completo y balanceado.
...
Write-Host "" ; Write-Host "==> Fix llave faltante en partes/[id]/page.tsx" -ForegroundColor Cyan
...
Write-Host "    OK: page.tsx escrito correctamente" -ForegroundColor Green
...
if ($opens -eq $closes) {
    Write-Host "    OK: llaves balanceadas ($opens = $closes)" -ForegroundColor Green
} else {
    Write-Host "    ERROR: llaves desbalanceadas (opens=$opens closes=$closes)" -ForegroundColor Red
}
...
if ($ok2) { Write-Host "    OK: sin materiales" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: firma cliente al final" -ForegroundColor Green } else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add "src\app\partes\[id]\page.tsx"'
Write-Host '  git commit -m "fix: llave faltante en ParteDetallePage - error compilacion resuelto"'
Write-Host '  git push'

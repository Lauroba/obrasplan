...
# deploy-georadar-v2-mejoras.ps1
# 1. "Hueco" renombrado a "Anomalia" en toda la UI y el informe Word
# 2. Mapa de calor Canvas sobre Google Maps (toggle Mapa / Ambos / Calor)
# 3. generateInformeDocx: typeLabel void=Anomalia, tuberia=Tuberia
...
$ok1 = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern '"Hueco"' -Quiet)
$ok2 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern 'drawHeatmap' -Quiet
$ok3 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern 'viewMode' -Quiet
if ($ok1) { Write-Host "    OK: sin referencias a 'Hueco'" -ForegroundColor Green }
else { Write-Host "    WARN: quedan referencias a Hueco" -ForegroundColor Yellow }
if ($ok2) { Write-Host "    OK: mapa de calor drawHeatmap presente" -ForegroundColor Green }
else { Write-Host "    ERROR: heatmap" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: toggle mapa/calor presente" -ForegroundColor Green }
else { Write-Host "    ERROR: toggle" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add -A'
Write-Host '  git commit -m "feat: Georadar V2 - mapa calor, Anomalia en vez de Hueco, informe mejorado"'
Write-Host '  git push'

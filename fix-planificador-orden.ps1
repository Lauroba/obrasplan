...
# fix-planificador-orden.ps1
# El reordenado de filas (drag de obras y RRHH) estaba bloqueado por
# el guard "if (!puedeAsignar) return" que se ejecutaba ANTES de
# cualquier logica, incluyendo el reordenado.
# Fix: el guard puedeAsignar solo se aplica a los bloques de insercion
# de asignaciones (drag de recurso a celda), no al reordenado de filas.
# El reordenado guarda orden_gantt y orden_planificacion en BD.
...
$ok = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "if \(!puedeAsignar\) return" -Quiet
if ($ok) { Write-Host "    OK: guards en bloques de asignacion" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "fix: reordenado planificador no bloqueado por guard puedeAsignar"'
Write-Host '  git push'

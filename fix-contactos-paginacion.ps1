...
# fix-contactos-leyna-paginacion.ps1
# 1. Corrige el limite de 1000 filas de Supabase API (sin limit explicito
#    la API devuelve maximo 1000, por eso solo se veian 1000 de 1675)
# 2. Aniade selector de cantidad: 10 / 50 / 100 / 500 / Todos
# 3. La cabecera muestra "X mostrados de Y contactos" para que se vea
#    claramente cuantos hay en total vs cuantos se muestran ahora
...
$ok = Select-String -Path "src\app\maestros\contactos-leyna\page.tsx" -Pattern "pageSize" -Quiet
if ($ok) { Write-Host "    OK: selector de cantidad presente" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\maestros\contactos-leyna\page.tsx'
Write-Host '  git commit -m "fix: contactos leyna - limit Supabase corregido, selector de cantidad"'
Write-Host '  git push'

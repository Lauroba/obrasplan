...
# fix-rrhh-guardar-rol.ps1
# El guardado del rol en RRHH fallaba silenciosamente cuando habia
# perfiles duplicados en public.users (mismo recurso_id en 2 filas).
# .single() devuelve null con duplicados -> el update del rol no se ejecutaba.
# Fix: usar query con limit+order para tomar el perfil mas reciente.
...
Write-Host "" ; Write-Host "==> Escribiendo api/users/route.ts" -ForegroundColor Cyan
...
$ok = Select-String -Path "src\app\api\users\route.ts" -Pattern "tolerante a duplicados" -Quiet
if ($ok) { Write-Host "    OK: query tolerante a duplicados" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\api\users\route.ts'
Write-Host '  git commit -m "fix: guardar rol en RRHH tolerante a duplicados de users"'
Write-Host '  git push'

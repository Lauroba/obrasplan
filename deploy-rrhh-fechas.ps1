...
# deploy-rrhh-fechas.ps1
# Aniade fecha_inicio y fecha_fin a recursos humanos:
#   - Formulario RRHH: campos editables fecha_inicio y fecha_fin
#   - Al desactivar acceso: fecha_fin = hoy automaticamente
#   - Planificador: RRHH solo visible entre sus fechas de disponibilidad
#   - SIN ASIGNAR: mismo filtro por dia
#   - Vista RRHH: filtra por rango de la semana visible
# IMPORTANTE: ejecutar primero 038_rrhh_fechas.sql en Supabase
...
export default function PlanificacionPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [oR, aR, hR, vR, eR] = await Promise.all([
      ...
      // Las fechas fecha_inicio y fecha_fin vienen incluidas en el select("*")
  ...
  const displayGrid = useMemo(() => {
    ...
    // Solo si el dia esta dentro del rango fecha_inicio..fecha_fin del recurso
    rrhh.filter((r) => (r as any).asignable !== false).forEach((person) => {
      weekDays.forEach((ds) => {
        const fi = (person as any).fecha_inicio;
        const ff = (person as any).fecha_fin;
        const disponibleEseDia = (!fi || ds >= fi) && (!ff || ds <= ff);
        if (!disponibleEseDia) return;
  ...
  const getResourceList = (tipo: RecursoTipo) => {
    ...
    if (tipo === "humano") return rrhh.filter((r) => {
      const fi = (r as any).fecha_inicio; const ff = (r as any).fecha_fin;
      return (r as any).asignable !== false && (!fi || todayDs >= fi) && (!ff || todayDs <= ff);
  ...
  const obrasPanelItems = useMemo(() => {
    ...
    if (resourceFilter === "all" || resourceFilter === "humano") rrhh.filter((r) => {
      const fi = (r as any).fecha_inicio;
      const ff = (r as any).fecha_fin;
      return (r as any).asignable !== false && (!fi || todayStr >= fi) && (!ff || todayStr <= ff);
    }).forEach((r) => all.push({ dragId: `res-humano|${r.id}`, nombre: r.nombre, foto_url: r.foto_url, detail: r.perfil || undefined, count: asignaciones.filter((a) => a.recurso_tipo === "humano" && a.recurso_id === r.id).length, iconType: "humano" }));
            ...
            <div className="flex-1 card overflow-hidden flex flex-col min-w-0">
              <div className="flex-1 overflow-auto">
                <div style={{ minWidth: LABEL_W + daysN * dw }}>
                  ...
                  {/* ===== VISTA RRHH ===== */}
                  {planView === "rrhh" && (() => {
                    // Filtrar RRHH por rango de fechas: solo los disponibles
                    // en algun dia del rango visible (viewStart..viewEnd)
                    const viewStart = dateStrs[0] || new Date().toISOString().slice(0, 10);
                    const viewEnd = dateStrs[dateStrs.length - 1] || viewStart;
                    const assignableRrhh = rrhh.filter((r) => {
                      if ((r as any).asignable === false) return false;
                      const fi = (r as any).fecha_inicio;
                      const ff = (r as any).fecha_fin;
                      // Disponible si su rango se solapa con el rango visible
                      return (!fi || viewEnd >= fi) && (!ff || viewStart <= ff);
                    });
...
$ok1 = Select-String -Path "src\app\maestros\recursos-humanos\page.tsx" -Pattern "fecha_inicio" -Quiet
$ok2 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "disponibleEseDia" -Quiet
if ($ok1) { Write-Host "    OK: formulario RRHH con fechas" -ForegroundColor Green }
else { Write-Host "    ERROR: falta fecha_inicio en formulario" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: planificador filtra por disponibilidad" -ForegroundColor Green }
else { Write-Host "    ERROR: falta filtro de fechas en planificador" -ForegroundColor Red }
Write-Host ""
Write-Host "RECORDATORIO: ejecutar 038_rrhh_fechas.sql en Supabase" -ForegroundColor Yellow
Write-Host ""
Write-Host '  git add src\app\maestros\recursos-humanos\page.tsx src\app\planificacion\page.tsx'
Write-Host '  git commit -m "feat: fecha_inicio y fecha_fin en RRHH, disponibilidad en planificador"'
Write-Host '  git push'

...
# fix-planificador-conflictos.ps1
# Cambia el comportamiento de conflictos en el planificador:
#   ANTES: se ponía en rojo el fondo de la celda obra×día entera
#   AHORA: solo el avatar del RRHH o vehículo en conflicto se pone en rojo
#
# Detalles:
#   - conflictCells (Set<"obraId|date">) → conflictResources (Set<"tipo|recursoId">)
#   - El Set ahora contiene el recurso, no la celda
#   - ObraCell recibe conflictResources y aplica ring-2 ring-red-500 solo
#     al avatar del recurso que tiene conflicto
#   - Tooltip del avatar muestra "⚠ Asignado a otra obra este día"
#   - La celda fondo NO cambia de color
#   - El badge de conflictos en la cabecera ahora muestra "N conflictos"
#     (N = número de recursos distintos en conflicto, no celdas)
...
Write-Host "" ; Write-Host "==> Fix conflictos planificador" -ForegroundColor Cyan
...
$ok1 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "conflictResources" -Quiet
$ok2 = !(Select-String -Path "src\app\planificacion\page.tsx" -Pattern "hasConflict" -Quiet)
$ok3 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "ring-red-500" -Quiet
if ($ok1) { Write-Host "    OK: conflictResources presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta conflictResources" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: hasConflict eliminado" -ForegroundColor Green }
else { Write-Host "    WARN: queda hasConflict" -ForegroundColor Yellow }
if ($ok3) { Write-Host "    OK: ring-red-500 en avatares" -ForegroundColor Green }
else { Write-Host "    ERROR: falta ring-red en avatares" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx'
Write-Host '  git commit -m "fix: planificador - conflicto marca solo el recurso, no la celda"'
Write-Host '  git push'

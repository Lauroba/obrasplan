      ...
      <div className="max-w-5xl mx-auto animate-fade-in">
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <>
                {puedeEditar && <button onClick={handleArchive} className={cn("flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg", obra.archivada ? "text-blue-700 bg-blue-50 hover:bg-blue-100" : "text-amber-700 bg-amber-50 hover:bg-amber-100")}>
                  {obra.archivada ? <Eye className="w-3.5 h-3.5" /> : <Archive className="w-3.5 h-3.5" />}
                  {obra.archivada ? "Desarchivar" : "Archivar"}
                </button>}
                {puedeEliminar && <button onClick={handleDelete} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100">
                  <Trash2 className="w-3.5 h-3.5" /> Eliminar
                </button>}
                {puedeEditar && <Link href={`/obras/nueva?edit=${id}`} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-surface-600 bg-surface-100 rounded-lg hover:bg-surface-200">
        ...
        {tab === "general" && (
          <div className="space-y-6">
            ...
            <div className="card p-6">
              ...
              <textarea value={observaciones} onChange={(e) => { setObservaciones(e.target.value); setObsChanged(true); }} rows={4} placeholder="Notas, comentarios..." disabled={!puedeEditar} className={ic + " resize-y" + (!puedeEditar ? " bg-surface-50 cursor-not-allowed" : "")} />
            </div>
          </div>
        )}

...
# fix-logs-entidades-v2.ps1
# 1. Entidades: fetch con limit 5000 y order para obtener todas las disponibles
# 2. Diff: mensaje claro cuando el log es antiguo y no tiene valor_anterior/nuevo
#    (los logs anteriores a la migracion 043b no tienen detalle de campos)
...
$ok = Select-String -Path "src\app\logs\page.tsx" -Pattern "limit.5000." -Quiet
if ($ok) { Write-Host "    OK: entidades con limit 5000" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\logs\page.tsx'
Write-Host '  git commit -m "fix: logs entidades dinamicas completas, mensaje sin diff"'
Write-Host '  git push'

...
# deploy-logs-permisos.ps1
# 1. Planificador: read-only para Encargado y Operario (sin puedeAsignar)
# 2. Obras detalle: nueva pestana Logs con historico de acciones de la obra
# IMPORTANTE: ejecutar 043_logs_por_obra_y_permisos.sql en Supabase PRIMERO
...
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText((Join-Path $RepoPath $dst), $content, $utf8NoBom)
Write-Host "    Escrito: src\app\obras\[id]\page.tsx" -ForegroundColor Green

Write-Host ""
Write-Host "==> Verificando" -ForegroundColor Cyan
$ok1 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "puedeAsignar" -Quiet
$ok2 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\obras\[id]\page.tsx") -Pattern "fetchLogs" -Quiet
if ($ok1) { Write-Host "    OK: planificador read-only para encargado" -ForegroundColor Green }
else { Write-Host "    ERROR planificador" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: pestana Logs en obras" -ForegroundColor Green }
else { Write-Host "    ERROR obras" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add -A'
Write-Host '  git commit -m "feat: logs por obra, permisos corregidos, planificador read-only encargado"'
Write-Host '  git push'

...
# fix-planificador-obras-3cambios.ps1
# 3 cambios en 2 archivos:
#
# 1. PLANIFICADOR - Conflicto sin rojo/negrita:
#    El avatar del recurso en conflicto mantiene su color original
#    (violeta para personas, color del tipo para vehiculos/maquinaria)
#    y solo muestra un anillo fino ring-2 ring-red-400 ring-offset-1
#    Sin bg-red-500 ni text-white
#
# 2. PLANIFICADOR - Espacio optimizado:
#    LABEL_W: 210 -> 185px (columna de obras mas estrecha)
#    DAY_WIDTHS week: 110 -> 96px (mas dias visibles sin scroll)
#    DAY_WIDTHS month: 40 -> 36px
#    min-h filas: 44px -> 38px (filas mas compactas)
#    padding handles drag: py-3 -> py-2
#    padding label obra: py-1.5 -> py-1
#
# 3. OBRAS - Selector activas/archivadas:
#    Tres botones "Activas / Archivadas / Todas" encima del listado
#    Por defecto muestra solo activas (no archivadas)
#    Los botones archivar/restaurar por linea se mantienen igual
#    El titulo de la tabla cambia dinamicamente segun el filtro
...
Write-Host "" ; Write-Host "==> Aplicando 3 mejoras" -ForegroundColor Cyan
...
$ok1 = !(Select-String -Path "src\app\planificacion\page.tsx" -Pattern "bg-red-500" -Quiet)
$ok2 = Select-String -Path "src\app\planificacion\page.tsx" -Pattern "LABEL_W = 185" -Quiet
$ok3 = Select-String -Path "src\app\obras\page.tsx" -Pattern "archivedFilter" -Quiet
if ($ok1) { Write-Host "    OK: sin bg-red-500 en conflictos" -ForegroundColor Green }
else { Write-Host "    WARN: queda bg-red-500" -ForegroundColor Yellow }
if ($ok2) { Write-Host "    OK: LABEL_W = 185" -ForegroundColor Green }
else { Write-Host "    ERROR: LABEL_W" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: selector archivadas en obras" -ForegroundColor Green }
else { Write-Host "    ERROR: falta archivedFilter" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\planificacion\page.tsx src\app\obras\page.tsx'
Write-Host '  git commit -m "fix: conflicto sin rojo negrita, planificador compacto, filtro obras archivadas"'
Write-Host '  git push'

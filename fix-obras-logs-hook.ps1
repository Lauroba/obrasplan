...
# fix-obras-logs-hook.ps1
# Corrige el error "client-side exception" en obras/[id]:
# el useEffect de fetchLogs estaba despues de returns condicionales,
# violando la regla de hooks de React (hooks siempre antes de cualquier return).
...
$ok = Select-String -LiteralPath (Join-Path $RepoPath "src\app\obras\[id]\page.tsx") -Pattern "fetchLogs" -Quiet
if ($ok) { Write-Host "    OK: fetchLogs presente y hooks en orden correcto" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add -A'
Write-Host '  git commit -m "fix: useEffect hooks antes de returns condicionales en obras/[id]"'
Write-Host '  git push'

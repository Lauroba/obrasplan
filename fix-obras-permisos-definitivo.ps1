...
# fix-obras-permisos-definitivo.ps1
# FIX DEFINITIVO: solo los operarios puros ven obras filtradas por asignacion.
# Roles como Oficina, Jefe Almacen, Jefe de Obra ven TODAS las obras.
#
# Problema raiz: filtrarObrasVisiblesOperario se aplicaba a CUALQUIER
# usuario no-admin con recurso_id, incluyendo Oficina, Almacen, etc.
#
# Fix: usa usePermissions para saber si el rol tiene crear/editar en obras.
# Si puede crear o editar -> ve todas las obras.
# Guard: espera a que permisosLoaded=true antes de fetchData para evitar
# la race condition que causaba que las obras desaparecieran 1 segundo
# despues de cargar.
...
$ok2 = Select-String -Path "src\app\obras\page.tsx" -Pattern "esSoloOperario" -Quiet
if ($ok1) { Write-Host "    OK: guard permisosLoaded presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta permisosLoaded" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: esSoloOperario presente" -ForegroundColor Green }
else { Write-Host "    ERROR: falta esSoloOperario" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\obras\page.tsx'
Write-Host '  git commit -m "fix: obras listado - solo operarios puros ven obras filtradas"'
Write-Host '  git push'

...
# fix-georadar-v2-gpspoints.ps1
# Corrige el crash al entrar en Georadar V2:
# MapsPanelV2 usaba store.gpsPoints pero el store tiene store.gps
# -> TypeError en runtime -> "client-side exception"
...
Write-Host "" ; Write-Host "==> Corrigiendo MapsPanelV2.tsx" -ForegroundColor Cyan
...
$ok = !(Select-String -LiteralPath (Join-Path $RepoPath "src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx") -Pattern "gpsPoints" -Quiet)
if ($ok) { Write-Host "    OK: store.gpsPoints -> store.gps corregido" -ForegroundColor Green }
else { Write-Host "    ERROR: quedan referencias a gpsPoints" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add src\app\aplicaciones\georadar-v2\MapsPanelV2.tsx'
Write-Host '  git commit -m "fix: store.gpsPoints -> store.gps en MapsPanelV2"'
Write-Host '  git push'

...
# fix-georadar-fullscreen-layout.ps1
# Corrige el boton de pantalla completa del georadar: el visor ahora ocupa
# toda la ventana correctamente al maximizar.
...
Write-Host ""
Write-Host "==> Escribiendo page.tsx corregido" -ForegroundColor Cyan
...
$f = "src\app\aplicaciones\georadar\page.tsx"
$ok = Select-String -Path $f -Pattern "georadar-container" -Quiet -ErrorAction SilentlyContinue
if ($ok) {
    Write-Host "    OK: correccion aplicada correctamente" -ForegroundColor Green
} else {
    Write-Host "    ERROR: no se encontro el marcador esperado" -ForegroundColor Red
}

Write-Host ""
Write-Host "Siguiente paso:" -ForegroundColor Green
Write-Host '  git add src\app\aplicaciones\georadar\page.tsx'
Write-Host '  git commit -m "fix: fullscreen georadar ocupa toda la ventana"'
Write-Host '  git push'

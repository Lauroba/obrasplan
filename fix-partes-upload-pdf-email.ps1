...
# fix-partes-upload-pdf-email.ps1
# 3 mejoras:
#
# 1. UPLOAD DE IMAGENES (page.tsx):
#    BUG: nombres con acentos, espacios, ñ eran rechazados por Supabase Storage
#    FIX: normalizar nombre antes de subir (quitar acentos, reemplazar especiales
#         por guiones bajos). El nombre original se mantiene en BD para mostrarlo.
#    EXTRA: input file movido fuera de contenedores flex para garantizar que
#    funciona desde galeria en iOS/Android (el onChange llegaba cortado dentro
#    de un contenedor flex con overflow hidden).
#
# 2. PDF (generatePartePdf.ts):
#    - Cabecera: PARTE DE TRABAJO + Obra (rojo, grande) + Fecha + Creado por
#      Todo en la primera línea del documento, muy visible
#    - Sección DATOS: ya no repite el nombre de la obra (estaba duplicado)
#    - Solo dirección, localidad y provincia en esa sección
#
# 3. EMAIL (route.ts):
#    - Asunto: "Parte: [Obra] | [Fecha] | [Creador]"
#    - Cuerpo: incluye "Creado por: [nombre]"
#    - Query del parte ahora incluye creator:users(nombre)
...
Write-Host "" ; Write-Host "==> Fix upload + PDF + email partes" -ForegroundColor Cyan
...
$ok1 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\partes\[id]\page.tsx") -Pattern "safeName" -Quiet
$ok2 = Select-String -LiteralPath (Join-Path $RepoPath "src\lib\pdf\generatePartePdf.ts") -Pattern "Creado por:" -Quiet
$ok3 = Select-String -LiteralPath (Join-Path $RepoPath "src\app\api\partes\email\route.ts") -Pattern "creador" -Quiet
if ($ok1) { Write-Host "    OK: safeName en upload" -ForegroundColor Green }
else { Write-Host "    ERROR: falta safeName" -ForegroundColor Red }
if ($ok2) { Write-Host "    OK: creador en cabecera PDF" -ForegroundColor Green }
else { Write-Host "    ERROR: falta creador en PDF" -ForegroundColor Red }
if ($ok3) { Write-Host "    OK: creador en asunto email" -ForegroundColor Green }
else { Write-Host "    ERROR: falta creador en email" -ForegroundColor Red }
Write-Host ""
Write-Host '  git add "src\app\partes\[id]\page.tsx" src\lib\pdf\generatePartePdf.ts src\app\api\partes\email\route.ts'
Write-Host '  git commit -m "fix: upload fotos galeria, PDF cabecera obra+creador, email asunto completo"'
Write-Host '  git push'

...
# fix-logs-entidades-v3.ps1
# 1. Usuarios: ya no se duplican - carga en useEffect separado (una vez)
# 2. Entidades: usa RPC get_audit_entidades() con SELECT DISTINCT real
#    Fallback automatico si la RPC no existe
# IMPORTANTE: ejecutar crear_rpc_entidades.sql en Supabase primero
...
Write-Host "" ; Write-Host "==> Escribiendo logs/page.tsx" -ForegroundColor Cyan

$dst = "src\app\logs\page.tsx"
$content = @'
"use client";
...
$ok = Select-String -Path "src\app\logs\page.tsx" -Pattern "get_audit_entidades" -Quiet
if ($ok) { Write-Host "    OK: RPC get_audit_entidades" -ForegroundColor Green }
else { Write-Host "    ERROR" -ForegroundColor Red }
Write-Host ""
Write-Host "RECORDATORIO: ejecutar crear_rpc_entidades.sql en Supabase" -ForegroundColor Yellow
Write-Host ""
Write-Host '  git add src\app\logs\page.tsx'
Write-Host '  git commit -m "fix: logs entidades DISTINCT via RPC, usuarios sin duplicar"'
Write-Host '  git push'
